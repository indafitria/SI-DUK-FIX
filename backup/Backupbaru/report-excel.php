...
$mergedCellStyles = $sheet->getStyle('A5:Y5');
$mergedCellStyles->applyFromArray([
    'font' => [
        'bold' => true,
        'name' => 'Times New Roman',
...
$mergedCellStyles = $sheet->getStyle('A6:Y6');
$mergedCellStyles->applyFromArray([
    'font' => [
        'bold' => true,
        'name' => 'Times New Roman',
...
$mergedCellStyles = $sheet->getStyle('A7:Y7');
$mergedCellStyles->applyFromArray([
    'font' => [
        'bold' => true,
        'name' => 'Times New Roman',
...
$sheet->setCellValue('Y5', 'CATATAN MUTASI');
$sheet->getStyle('Y5')
    ->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER)
    ->SetVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER)
    ->setWrapText(true);
...
$query = "SELECT * FROM pegawai JOIN data_pribadi ON pegawai.id_data_pribadi = data_pribadi.id_data_pribadi JOIN golongan ON pegawai.id_golongan = golongan.id_golongan JOIN data_bidang ON pegawai.id_bidang = data_bidang.id_bidang";
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $sheet->setCellValue('C' . $startRow, $nip);
    ...
    while ($pend = mysqli_fetch_array($resultMutasi)) {
        ...
        $sheet->setCellValue('Y' . $startRow, $catatan_mutasi);
...
$sheet->getStyle('A' . $endRow . ':' . 'Y' . $endRow)
    ->getBorders()
    ->getBottom()
    ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK);
    ...
    $sheet->setCellValue('Y' . $rowTTD, 'Palangka Raya, ' . $tanggalHariIni1);
    $sheet->setCellValue('Y' . $rowTTD_1, 'Yang membuat,');
    $sheet->setCellValue('Y' . $rowTTD_2, 'Agus Siswadi');
    $sheet->setCellValue('Y' . $rowTTD_3, '19680204 199903 1 007');
    
    $sheet->getStyle('Y' . $rowTTD. ':' . 'Y' . $rowTTD_3)
    ->getFont()
    ->setName('Times New Roman')
    ->setSize(10);
