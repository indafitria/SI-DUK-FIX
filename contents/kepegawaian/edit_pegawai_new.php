...
if (isset($_POST['simpan_edp'])) {
    ...
    if (empty($lokasi_foto) && empty($lokasi_sk) && empty($lokasi_ijazah) && empty($lokasi_sk_pangkat)) {
        ...
        ubahDataPegawai($nip, $id_bidang, $id_golongan, $status, $npwp, $tmt_gol, $nomor_sk_golongan, $jab, $tmt_jab, $no_sk_jabatan, $mkt, $mkb);
        ...
        if (!empty($lokasi_foto)) {
            ...
            if ($ukuran_foto > 15000000) { // 15 MB dalam bytes
                ...
                $allowed_extensions = array('jpg', 'jpeg', 'JPEG', 'JPG', 'PNG', 'png');
        ...
        if (!empty($lokasi_sk) && !empty($lokasi_sk_pangkat)) {
            ...
            if ($ukuran_sk > 15000000 || $ukuran_sk_pangkat >15000000) { // 15 MB dalam bytes
                echo "<script> alert('Ukuran file lebih dari 15 MB, data tidak dapat ditambahkan.'); window.history.back();</script>";
                // exit();
            } else if ($ukuran_sk < 15000000 || $ukuran_sk_pangkat < 15000000 ) {
                $allowed_extensions = array('pdf');
                $file_extension = pathinfo($sk, PATHINFO_EXTENSION);
                $file_extension_2 = pathinfo($sk_pangkat, PATHINFO_EXTENSION);

                if (!in_array(strtolower($file_extension), $allowed_extensions) || !in_array(strtolower($file_extension_2), $allowed_extensions)) {
                    echo "<script> alert('Format file SK tidak valid. Hanya file PDFG yang diizinkan.'); window.history.back();</script>";
                    // exit();
                } else {
                    move_uploaded_file($lokasi_sk, '../../Penyimpanan/sk/' . $sk);
                    move_uploaded_file($lokasi_sk, '../../Penyimpanan/sk/' . $sk_pangkat);
                    $hasil_2 = ubahDataPegawai1($nip,$id_bidang,$id_golongan,$status, $npwp, $tmt_gol,$nomor_sk_golongan,$sk_pangkat,   
                               $jab, $tmt_jab, $no_sk_jabatan, $sk, $mkt,$mkb);
                    $hasil_3 = tambahMutasi($nip,$cttn_mutasi1, $no_sk_jabatan,$tmt_jabatan,$sk);
                    $hasil_4 = tambahPangkat($nip, $jenis_sk,$nama_golongan,$tmt_gol,$nomor_sk_golongan,$sk_pangkat);
                }
            } else {
        ...
        if (!empty($lokasi_sk) && !empty($lokasi_sk_pangkat)) {
            ...
            if ($ukuran_sk > 15000000 || $ukuran_sk_pangkat > 15000000) { // 15 MB dalam bytes
                ...
                $file_extension = pathinfo($lokasi_sk, PATHINFO_EXTENSION);
                $file_extension_2 = pathinfo($lokasi_sk_pangkat, PATHINFO_EXTENSION);

                if (!in_array(strtolower($file_extension), $allowed_extensions) || !in_array(strtolower($file_extension_2), $allowed_extensions)) {
                    echo "<script> alert('Format file SK kedua tidak valid. Hanya file PDF yang diizinkan.'); window.history.back();</script>";
                    // exit();
                } else {
            ...
            if ($ukuran_sk > 15000000) { // 15 MB dalam bytes
                ...
                if (!in_array(strtolower($file_extension), $allowed_extensions)) {
                    echo "<script> alert('Format file sk jabatan tidak valid. Hanya file PDF yang diizinkan.'); window.history.back();</script>";
                    // exit();
                } else {
                    // Move the uploaded files to their respective directories
                    move_uploaded_file($lokasi_sk, '../../Penyimpanan/sk/' . $sk);
                    ...
                    if ($ukuran_sk < 15000000 && $ukuran_sk_pangkat < 15000000) {
                        $hasil_2 = ubahDataPegawai2($nip, $id_bidang, $id_golongan, $status, $npwp, $tmt_gol, $nomor_sk_golongan, $jab, $tmt_jab, $no_sk_jabatan, $sk, $mkt, $mkb);
            ...
            if ($ukuran_sk_pangkat > 15000000) { // 15 MB dalam bytes
                echo "<script> alert('Ukuran file lebih dari 15 MB, data tidak dapat ditambahkan.'); window.history.back();</script>";
                exit();
            } else {
                $allowed_extensions = array('PDF','pdf');
                $file_extension = pathinfo($lokasi_sk_pangkat, PATHINFO_EXTENSION);

                if (!in_array(strtolower($file_extension), $allowed_extensions)) {
                    echo "<script> alert('Format file sk pangkat tidak valid. Hanya file PDF yang diizinkan.'); window.history.back();</script>";
                    exit();
                } else {
                    // Move the uploaded files to their respective directories
                    move_uploaded_file($lokasi_sk_pangkat, '../../Penyimpanan/sk/' . $sk_pangkat);
                    ...
                    if ($ukuran_sk < 15000000 && $ukuran_sk_pangkat < 15000000) {
                        ...
                        $hasil_3 = tambahPangkat($nip, $jenis_sk, $nama_golongan, $tmt_gol, $nomor_sk_golongan, $sk_pangkat);
                    } else {
                        $hasil_2 = ubahDataPegawai($nip, $id_bidang, $id_golongan, $status, $npwp, $tmt_gol, $jab, $tmt_jab, $no_sk_jabatan, $mkt, $mkb);
                    }
                }
